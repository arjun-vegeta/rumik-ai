...
export default function TryIraEmbed() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#FFF8E1] rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
      {/* Chat Header */}
      <div className="p-4 bg-black">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium ml-2 text-white text-2xl" style={{ fontFamily: 'Young Serif, serif' }}>Ira</p>
            ...
            >
              <div
                className={`max-w-[80%] px-4 py-3 text-sm ${
                  message.role === "user"
                    ? "bg-[#eee090] text-black rounded-2xl rounded-br-[2px]"
                    : "bg-black text-[#E5E0CD] rounded-2xl rounded-bl-[2px]"
                }`}
              >
                {message.content}
              </div>
        ...
        {messages.filter(m => m.role === "user").length >= 6 ? (
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-3 rounded-full border border-gray-300 focus:outline-none focus:border-black transition-colors text-base bg-white"
              disabled={isLoading}
            />
            <button
              onClick={handleSend}
